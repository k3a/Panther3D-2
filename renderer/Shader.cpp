...
namespace P3D
{
	...
	bool CShader::Create(sShaderDesc &desc)
	{
		m_compiled = false;
...
// 			m_vertDecl.Reserve(CRenderer::cGraphicsManager()->GetVertexDescCount());
// 			for (unsigned int i=0; i<CRenderer::cGraphicsManager()->GetVertexDescCount(); i++)
// 			{
// 				CVertexDeclaration *decl = 0;
// 				#if defined(_OGL)
// 				decl = new CVertexDeclaration(CRenderer::cGraphicsManager()->GetVertexDescByID(i), 0);
// 				#elif defined(_DX) && _DX==9
// 				decl = new CVertexDeclaration(CRenderer::cGraphicsManager()->GetVertexDescByID(i), 0);
// 				#elif defined(_DX) && _DX==10
// 				ID3D10Blob * buf = cgD3D10GetCompiledProgram(m_program);
// 				decl = new CVertexDeclaration(CRenderer::cGraphicsManager()->GetVertexDescByID(i), buf);
// 				//SAFE_RELEASE(buf); //I'm not sure whether I should do this
// 				#endif
// 				m_vertDecl.Add(decl);
// 			}
	...
	const char* CShader::GetCompiled()
	{
	...
	ShaderHandle CShader::GetHandle(const char *name)
	{
		assertd(m_compiled, "Shader must be compiled!");
	...
	void CShader::SetHandleValueVec3(P3D::ShaderHandle handle, const P3D::Vec3 *value)
	{
		cgSetParameter3fv(m_parameters[handle-1], &value->x);
	}

	void CShader::SetHandleValueTexture(P3D::ShaderHandle handle, P3D::ITexture *tex)
	{
		...
		#elif defined(_DX) && _DX==10
		//ID3D10Resource *res = (ID3D10Resource*)ctex->GetInternalTexture();
	...
	void CShader::SetHandleValueMatrix(ShaderHandle handle, const Matrix *matrix)
	{
		//cgSetMatrixParameterfr(m_parameters[handle-1], (float*)matrix);
		cgSetMatrixParameterfc(m_parameters[handle-1], (float*)matrix);
	}

	void CShader::SetHandleFloatArray(P3D::ShaderHandle handle, const float *farray, unsigned int numelements)
	{
		cgSetParameterValuefc(CheckArrayIndex(m_parameters[handle-1]), numelements, farray);
	}

	CGparameter CShader::CheckArrayIndex(CGparameter param)
	{
	...
	void CShader::SetBuffer(CShaderBuffer *buffer, ShaderHandle handle)
	{
