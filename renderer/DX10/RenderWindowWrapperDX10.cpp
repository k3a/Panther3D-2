...
namespace P3D
{
	...
	CRenderWindowWrapper::CRenderWindowWrapper()
	{
		m_pSwapChain = NULL;
	...
	bool CRenderWindowWrapper::Create(const sRenderWindowDesc &desc, void *winHandle)
	{
		...
		g_pD3ddev->CreateRenderTargetView(backBuffer, 0, &m_pRenderTargetView);
		backBuffer->Release();

		D3D10_TEXTURE2D_DESC depthStencilDesc;
		depthStencilDesc.Width     = desc.size.x;
		depthStencilDesc.Height    = desc.size.y;
		...
		g_pD3ddev->CreateTexture2D(&depthStencilDesc, 0, &depthBuffer);
		g_pD3ddev->CreateDepthStencilView(depthBuffer, 0, &m_pDepthStencil);
		depthBuffer->Release();
